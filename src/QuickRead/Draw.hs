...
drawUI reader =
  [ BC.hCenter (drawFileQueue $ reader ^. fileQueue)
      <=> textViewport
      <=> curWpm
      <=> speed
      <=> drawProgress reader
...
drawFileQueue :: Maybe (T.Tape String) -> B.Widget Name
drawFileQueue Nothing                          = B.str "Empty File Queue"
drawFileQueue (Just (T.Tape left focus right)) = BB.border $ F.foldr1
  (<+>)
  [ F.foldr (<+>) B.emptyWidget $ B.str <$> L.intersperse "|" (F.toList left)
  , BB.border (B.str focus)
  , F.foldr (<+>) B.emptyWidget $ B.str <$> L.intersperse "|" (F.toList right)
  ]
...
drawProgress reader = BB.border bar
 where
  prog = case reader ^. textTape of
    Just (T.Tape l _ r) -> fromIntegral (succ $ S.length l)
      / succ (sum $ map (fromIntegral . S.length) [l, r])
    Nothing -> 1

  bar =
    B.updateAttrMap
        (B.mapAttrNames
          [ ("TEST"     , BP.progressCompleteAttr)
...
drawHighlight xs     = case Control.Lens.uncons mr of
